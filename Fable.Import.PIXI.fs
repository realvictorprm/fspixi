...
and [<AllowNullLiteral>] [<Import("*","Application")>] Application(?width: float, ?height: float, ?options: ApplicationOptions, ?noWebGL: bool, ?sharedTicker: bool, ?sharedLoader: bool) =
...
and [<AllowNullLiteral>] [<Import("*","Bounds")>] Bounds() =
...
and [<AllowNullLiteral>] [<Import("*","Container")>] Container() =
...
and [<AllowNullLiteral>] [<Import("*","DisplayObject")>] DisplayObject() =
...
and [<AllowNullLiteral>] [<Import("*","TransformBase")>] TransformBase() =
...
and [<AllowNullLiteral>] [<Import("*","TransformStatic")>] TransformStatic() =
...
and [<AllowNullLiteral>] [<Import("*","Transform")>] Transform() =
    inherit TransformBase()
...
and [<AllowNullLiteral>] [<Import("*","GraphicsData")>] GraphicsData(lineWidth: float, lineColor: float, lineAlpha: float, fillColor: float, fillAlpha: float, fill: bool, nativeLines: bool, shape: obj) =
...
and [<AllowNullLiteral>] [<Import("*","Graphics")>] Graphics(?nativeLines: bool) =
    inherit Container()
    member __.fillAlpha with get(): float = jsNative and set(v: float): unit = jsNative
    member __.lineWidth with get(): float = jsNative and set(v: float): unit = jsNative
    member __.nativeLines with get(): bool = jsNative and set(v: bool): unit = jsNative
...
and [<AllowNullLiteral>] [<Import("*","CanvasGraphicsRenderer")>] CanvasGraphicsRenderer(renderer: SystemRenderer) =
...
and [<AllowNullLiteral>] [<Import("*","GraphicsRenderer")>] GraphicsRenderer(renderer: PIXI.CanvasRenderer) =
...
and [<AllowNullLiteral>] [<Import("*","WebGLGraphicsData")>] WebGLGraphicsData(gl: WebGLRenderingContext, shader: glCore.GLShader, attribsState: glCore.AttribState) =
    member __.gl with get(): WebGLRenderingContext = jsNative and set(v: WebGLRenderingContext): unit = jsNative
...
and [<AllowNullLiteral>] [<Import("*","PrimitiveShader")>] PrimitiveShader() =
    inherit glCore.GLShader()


and [<AllowNullLiteral>] [<Import("*","Matrix")>] Matrix(?a: float, ?b: float, ?c: float, ?d: float, ?tx: float, ?ty: float) =
...
and [<AllowNullLiteral>] [<Import("*","ObservablePoint")>] ObservablePoint(cb: Func<unit, obj>, ?scope: obj, ?x: float, ?y: float) =
    member __.x with get(): float = jsNative and set(v: float): unit = jsNative
    member __.y with get(): float = jsNative and set(v: float): unit = jsNative
...
and [<AllowNullLiteral>] [<Import("*","Point")>] Point(?x: float, ?y: float) =
    member __.x with get(): float = jsNative and set(v: float): unit = jsNative
    member __.y with get(): float = jsNative and set(v: float): unit = jsNative
...
and [<AllowNullLiteral>] [<Import("*","Circle")>] Circle(?x: float, ?y: float, ?radius: float) =
    member __.x with get(): float = jsNative and set(v: float): unit = jsNative
    member __.y with get(): float = jsNative and set(v: float): unit = jsNative
...
and [<AllowNullLiteral>] [<Import("*","Ellipse")>] Ellipse(?x: float, ?y: float, ?width: float, ?height: float) =
    member __.x with get(): float = jsNative and set(v: float): unit = jsNative
    member __.y with get(): float = jsNative and set(v: float): unit = jsNative
    member __.width with get(): float = jsNative and set(v: float): unit = jsNative
    member __.height with get(): float = jsNative and set(v: float): unit = jsNative
...
and [<AllowNullLiteral>] [<Import("*","Polygon")>] Polygon([<ParamArray>] points: float[]) =
...
and [<AllowNullLiteral>] [<Import("*","Rectangle")>] Rectangle(?x: float, ?y: float, ?width: float, ?height: float) =
    member __.x with get(): float = jsNative and set(v: float): unit = jsNative
    member __.y with get(): float = jsNative and set(v: float): unit = jsNative
    member __.width with get(): float = jsNative and set(v: float): unit = jsNative
    member __.height with get(): float = jsNative and set(v: float): unit = jsNative
...
and [<AllowNullLiteral>] [<Import("*","RoundedRectangle")>] RoundedRectangle(?x: float, ?y: float, ?width: float, ?height: float, ?radius: float) =
    member __.x with get(): float = jsNative and set(v: float): unit = jsNative
    member __.y with get(): float = jsNative and set(v: float): unit = jsNative
    member __.width with get(): float = jsNative and set(v: float): unit = jsNative
    member __.height with get(): float = jsNative and set(v: float): unit = jsNative
...
and [<AllowNullLiteral>] [<Import("*","SystemRenderer")>] SystemRenderer(system: string, ?screenWidth: float, ?screenHeight: float, ?options: RendererOptions) =
    inherit utils.EventEmitter()
    member __.``type`` with get(): float = jsNative and set(v: float): unit = jsNative
...
and [<AllowNullLiteral>] [<Import("*","CanvasRenderer")>] CanvasRenderer(?screenWidth: float, ?screenHeight: float, ?options: RendererOptions) =
...
and [<AllowNullLiteral>] [<Import("*","CanvasMaskManager")>] CanvasMaskManager(renderer: CanvasRenderer) =
...
and [<AllowNullLiteral>] [<Import("*","CanvasRenderTarget")>] CanvasRenderTarget(width: float, height: float, resolution: float) =
...
and [<AllowNullLiteral>] WebGLRendererOptions =
    abstract view: HTMLCanvasElement option with get, set
    abstract transparent: bool option with get, set
    abstract autoResize: bool option with get, set
    abstract antialias: bool option with get, set
...
and [<AllowNullLiteral>] [<Import("*","WebGLRenderer")>] WebGLRenderer(?screenWidth: float, ?screenHeight: float, ?options: WebGLRendererOptions) =
    inherit SystemRenderer()
    member __.___plugins with get(): obj = jsNative and set(v: obj): unit = jsNative
    member __.plugins with get(): obj = jsNative and set(v: obj): unit = jsNative
...
and [<AllowNullLiteral>] [<Import("*","WebGLState")>] WebGLState(gl: WebGLRenderingContext) =
...
and [<AllowNullLiteral>] [<Import("*","TextureManager")>] TextureManager(renderer: WebGLRenderer) =
...
and [<AllowNullLiteral>] [<Import("*","TextureGarbageCollector")>] TextureGarbageCollector(renderer: WebGLRenderer) =
    member __.renderer with get(): WebGLRenderer = jsNative and set(v: WebGLRenderer): unit = jsNative
...
and [<AllowNullLiteral>] [<Import("*","ObjectRenderer")>] ObjectRenderer(renderer: WebGLRenderer) =
...
and [<AllowNullLiteral>] [<Import("*","Quad")>] Quad(gl: WebGLRenderingContext) =
    member __.gl with get(): WebGLRenderingContext = jsNative and set(v: WebGLRenderingContext): unit = jsNative
...
and [<AllowNullLiteral>] [<Import("*","RenderTarget")>] RenderTarget(gl: WebGLRenderingContext, width: float, height: float, scaleMode: float, resolution: float, ?root: bool) =
    member __.gl with get(): WebGLRenderingContext = jsNative and set(v: WebGLRenderingContext): unit = jsNative
...
and [<AllowNullLiteral>] [<Import("*","BlendModeManager")>] BlendModeManager(renderer: WebGLRenderer) =
    inherit WebGLManager()
...
and [<AllowNullLiteral>] [<Import("*","FilterManager")>] FilterManager(renderer: WebGLRenderer) =
    inherit WebGLManager()
    member __.gl with get(): WebGLRenderingContext = jsNative and set(v: WebGLRenderingContext): unit = jsNative
...
and [<AllowNullLiteral>] [<Import("*","StencilMaskStack")>] StencilMaskStack() =
...
and [<AllowNullLiteral>] [<Import("*","StencilManager")>] StencilManager(renderer: WebGLRenderer) =
    inherit WebGLManager()
    member __.stencilMaskStack with get(): ResizeArray<Graphics> = jsNative and set(v: ResizeArray<Graphics>): unit = jsNative
...
and [<AllowNullLiteral>] [<Import("*","WebGLManager")>] WebGLManager(renderer: WebGLRenderer) =
...
and [<AllowNullLiteral>] UniformData =
    abstract ``type``: string with get, set
    abstract value: obj with get, set
    abstract name: string option with get, set

and [<AllowNullLiteral>] [<Import("*","Filter")>] Filter(?vertexSrc: string, ?fragmentSrc: string, ?uniforms: obj) =
...
and [<AllowNullLiteral>] [<Import("*","SpriteMaskFilter")>] SpriteMaskFilter(sprite: Sprite) =
    inherit Filter()
...
and [<AllowNullLiteral>] [<Import("*","Sprite")>] Sprite(?texture: Texture) =
    inherit Container()
...
and [<AllowNullLiteral>] [<Import("*","BatchBuffer")>] BatchBuffer() =
...
and [<AllowNullLiteral>] [<Import("*","SpriteRenderer")>] SpriteRenderer(renderer: PIXI.WebGLRenderer) =
    inherit ObjectRenderer()
...
and [<AllowNullLiteral>] [<Import("*","CanvasSpriteRenderer")>] CanvasSpriteRenderer(renderer: WebGLRenderer) =
    inherit ObjectRenderer()
    member __.render(sprite: Sprite): unit = jsNative
    member __.destroy(): unit = jsNative
...
and [<AllowNullLiteral>] [<Import("*","TextStyle")>] TextStyle(style: TextStyleOptions) =
...
and [<AllowNullLiteral>] [<Import("*","TextMetrics")>] TextMetrics(text: string, style: TextStyle, width: float, height: float, lines: ResizeArray<float>, lineWidths: ResizeArray<float>, lineHeight: float, maxLineWidth: float, fontProperties: obj) =
...
and [<AllowNullLiteral>] [<Import("*","Text")>] Text(?text: string, ?style: TextStyleOptions, ?canvas: HTMLCanvasElement) =
    inherit Sprite()
    member __.canvas with get(): HTMLCanvasElement = jsNative and set(v: HTMLCanvasElement): unit = jsNative
    member __.context with get(): CanvasRenderingContext2D = jsNative and set(v: CanvasRenderingContext2D): unit = jsNative
    member __.resolution with get(): float = jsNative and set(v: float): unit = jsNative
...
and [<AllowNullLiteral>] [<Import("*","BaseRenderTexture")>] BaseRenderTexture(?width: float, ?height: float, ?scaleMode: float, ?resolution: float) =
    inherit BaseTexture()
    member __.height with get(): float = jsNative and set(v: float): unit = jsNative
    member __.width with get(): float = jsNative and set(v: float): unit = jsNative
...
and [<AllowNullLiteral>] [<Import("*","BaseTexture")>] BaseTexture(?source: U3<HTMLImageElement, HTMLCanvasElement, HTMLVideoElement>, ?scaleMode: float, ?resolution: float) =
    inherit utils.EventEmitter()
...
and [<AllowNullLiteral>] [<Import("*","RenderTexture")>] RenderTexture(baseRenderTexture: BaseRenderTexture, ?frame: Rectangle) =
    inherit Texture()
...
and [<AllowNullLiteral>] [<Import("*","Texture")>] Texture(baseTexture: BaseTexture, ?frame: Rectangle, ?orig: Rectangle, ?trim: Rectangle, ?rotate: float) =
    inherit utils.EventEmitter()
...
and [<AllowNullLiteral>] [<Import("*","TextureUvs")>] TextureUvs() =
...
and [<AllowNullLiteral>] [<Import("*","Spritesheet")>] Spritesheet(baseTexture: BaseTexture, data: obj, ?resolutionFilename: string) =
...
and [<AllowNullLiteral>] [<Import("*","VideoBaseTexture")>] VideoBaseTexture(source: HTMLVideoElement, ?scaleMode: float) =
    inherit BaseTexture()
...
and [<AllowNullLiteral>] [<Import("*","Shader")>] Shader() =
    inherit glCore.GLShader()


and [<AllowNullLiteral>] [<Import("*","MiniSignalBinding")>] MiniSignalBinding(fn: Function, ?once: bool, ?thisArg: obj) =
...
and [<AllowNullLiteral>] [<Import("*","MiniSignal")>] MiniSignal() =
...
and [<AllowNullLiteral>] DecomposedDataUri =
...
module PIXI =
    type [<Import("*","PIXI")>] Globals =
        ...
        static member PRECISION with get(): string = jsNative and set(v: string): unit = jsNative
...
module accessibility =
    type [<AllowNullLiteral>] [<Import("AccessibilityManager","accessibility")>] AccessibilityManager(renderer: U2<CanvasRenderer, WebGLRenderer>) =
...
module ticker =
    type [<AllowNullLiteral>] [<Import("TickerListener","ticker")>] TickerListener(fn: Func<float, unit>, ?context: obj, ?priority: float, ?once: bool) =
    ...
    and [<AllowNullLiteral>] [<Import("Ticker","ticker")>] Ticker() =
...
module extract =
    type [<AllowNullLiteral>] [<Import("CanvasExtract","extract")>] CanvasExtract(renderer: CanvasRenderer) =
    ...
    and [<AllowNullLiteral>] [<Import("WebGLExtract","extract")>] WebGLExtract(renderer: WebGLRenderer) =
        member __.renderer with get(): WebGLRenderer = jsNative and set(v: WebGLRenderer): unit = jsNative
        member __.image(?target: U2<DisplayObject, RenderTexture>): HTMLImageElement = jsNative
        member __.base64(?target: U2<DisplayObject, RenderTexture>): string = jsNative
        member __.canvas(?target: U2<DisplayObject, RenderTexture>): HTMLCanvasElement = jsNative
...
module extras =
    ...
    and [<AllowNullLiteral>] [<Import("BitmapText","extras")>] BitmapText(text: string, ?style: BitmapTextStyle) =
        interface Container
    ...
    and [<AllowNullLiteral>] [<Import("AnimatedSprite","extras")>] AnimatedSprite(textures: U2<ResizeArray<Texture>, ResizeArray<AnimatedSpriteTextureTimeObject>>, ?autoUpdate: bool) =
        interface Sprite
    ...
    and [<AllowNullLiteral>] [<Import("TextureTransform","extras")>] TextureTransform(texture: Texture, ?clampMargin: float) =
        member __._texture with get(): Texture = jsNative and set(v: Texture): unit = jsNative
    ...
    and [<AllowNullLiteral>] [<Import("TilingSprite","extras")>] TilingSprite(texture: Texture, ?width: float, ?height: float) =
        interface Sprite
    ...
    and [<AllowNullLiteral>] [<Import("TilingSpriteRenderer","extras")>] TilingSpriteRenderer(renderer: WebGLRenderer) =
...
module interaction =
    ...
    and [<AllowNullLiteral>] [<Import("InteractionData","interaction")>] InteractionData() =
    ...
    and [<AllowNullLiteral>] [<StringEnum>] InteractionPointerEvents =
        | Pointerdown | Pointercancel | Pointerup | Pointertap | Pointerupoutside | Pointermove | Pointerover | Pointerout

    and [<AllowNullLiteral>] [<StringEnum>] InteractionTouchEvents =
        | Touchstart | Touchcancel | Touchend | Touchendoutside | Touchmove | Tap

    and [<AllowNullLiteral>] [<StringEnum>] InteractionMouseEvents =
        | Rightdown | Mousedown | Rightup | Mouseup | Rightclick | Click | Rightupoutside | Mouseupoutside | Mousemove | Mouseover | Mouseout | Mouseover

    and [<AllowNullLiteral>] InteractionEventTypes =
        U3<InteractionPointerEvents, InteractionTouchEvents, InteractionMouseEvents>
    ...
    and [<AllowNullLiteral>] [<Import("InteractionManager","interaction")>] InteractionManager(renderer: U3<CanvasRenderer, WebGLRenderer, SystemRenderer>, ?options: InteractionManagerOptions) =
        inherit utils.EventEmitter()
        member __.renderer with get(): SystemRenderer = jsNative and set(v: SystemRenderer): unit = jsNative
        ...
        member __.processPointerUp with get(): Func<InteractionEvent, U3<Container, PIXI.Sprite, undefined.TilingSprite>, bool, unit> = jsNative and set(v: Func<InteractionEvent, U3<Container, PIXI.Sprite, undefined.TilingSprite>, bool, unit>): unit = jsNative
        member __.onPointerCancel with get(): Func<PointerEvent, unit> = jsNative and set(v: Func<PointerEvent, unit>): unit = jsNative
        member __.processPointerCancel with get(): Func<InteractionEvent, U3<PIXI.Container, PIXI.Sprite, undefined.TilingSprite>, unit> = jsNative and set(v: Func<InteractionEvent, U3<PIXI.Container, PIXI.Sprite, undefined.TilingSprite>, unit>): unit = jsNative
        member __.onPointerDown with get(): Func<PointerEvent, unit> = jsNative and set(v: Func<PointerEvent, unit>): unit = jsNative
        member __.processPointerDown with get(): Func<InteractionEvent, U3<PIXI.Container, PIXI.Sprite, undefined.TilingSprite>, bool, unit> = jsNative and set(v: Func<InteractionEvent, U3<PIXI.Container, PIXI.Sprite, undefined.TilingSprite>, bool, unit>): unit = jsNative
        member __.onPointerMove with get(): Func<PointerEvent, unit> = jsNative and set(v: Func<PointerEvent, unit>): unit = jsNative
        member __.processPointerMove with get(): Func<InteractionEvent, U3<PIXI.Container, PIXI.Sprite, undefined.TilingSprite>, bool, unit> = jsNative and set(v: Func<InteractionEvent, U3<PIXI.Container, PIXI.Sprite, undefined.TilingSprite>, bool, unit>): unit = jsNative
        member __.onPointerOut with get(): Func<PointerEvent, unit> = jsNative and set(v: Func<PointerEvent, unit>): unit = jsNative
        member __.processPointerOverOut with get(): Func<InteractionEvent, U3<PIXI.Container, PIXI.Sprite, undefined.TilingSprite>, bool, unit> = jsNative and set(v: Func<InteractionEvent, U3<PIXI.Container, PIXI.Sprite, undefined.TilingSprite>, bool, unit>): unit = jsNative
        ...
        member __.processInteractive(interactionEvent: InteractionEvent, displayObject: U3<PIXI.Container, PIXI.Sprite, undefined.TilingSprite>, ?func: Function, ?hitTest: bool, ?interactive: bool): bool = jsNative
...
module loaders =
    ...
    and [<AllowNullLiteral>] ResourceDictionary =
        [<Emit("$0[$1]{{=$2}}")>] abstract Item: index: string -> undefined.Resource with get, set

    and [<AllowNullLiteral>] [<Import("Loader","loaders")>] Loader(?baseUrl: string, ?concurrency: float) =
        inherit utils.EventEmitter()
    ...
    and [<AllowNullLiteral>] [<Import("Resource","loaders")>] Resource(name: string, url: U2<string, ResizeArray<string>>, ?options: LoaderOptions) =
...
module mesh =
    type [<AllowNullLiteral>] [<Import("Mesh","mesh")>] Mesh(texture: Texture, ?vertices: Float32Array, ?uvs: Float32Array, ?indices: Uint16Array, ?drawMode: float) =
        interface Container
        member __._texture with get(): Texture = jsNative and set(v: Texture): unit = jsNative
    ...
    and [<AllowNullLiteral>] [<Import("CanvasMeshRenderer","mesh")>] CanvasMeshRenderer(renderer: CanvasRenderer) =
        member __.renderer with get(): CanvasRenderer = jsNative and set(v: CanvasRenderer): unit = jsNative
    ...
    and [<AllowNullLiteral>] [<Import("MeshRenderer","mesh")>] MeshRenderer(renderer: WebGLRenderer) =
        interface ObjectRenderer
        member __.shader with get(): Shader = jsNative and set(v: Shader): unit = jsNative
        member __.render(mesh: Mesh): unit = jsNative

    and [<AllowNullLiteral>] [<Import("Plane","mesh")>] Plane(texture: Texture, ?verticesX: float, ?verticesY: float) =
        inherit Mesh()
    ...
    and [<AllowNullLiteral>] [<Import("NineSlicePlane","mesh")>] NineSlicePlane(texture: Texture, ?leftWidth: float, ?topHeight: float, ?rightWidth: float, ?bottomHeight: float) =
        inherit Plane()
        member __.width with get(): float = jsNative and set(v: float): unit = jsNative
        member __.height with get(): float = jsNative and set(v: float): unit = jsNative
    ...
    and [<AllowNullLiteral>] [<Import("Rope","mesh")>] Rope(texture: Texture, points: ResizeArray<Point>) =
        inherit Mesh()
        member __.points with get(): ResizeArray<Point> = jsNative and set(v: ResizeArray<Point>): unit = jsNative
...
module particles =
    ...
    and [<AllowNullLiteral>] [<Import("ParticleContainer","particles")>] ParticleContainer(?size: float, ?properties: ParticleContainerProperties, ?batchSize: float) =
        interface Container
        member __._tint with get(): float = jsNative and set(v: float): unit = jsNative
    ...
    and [<AllowNullLiteral>] [<Import("ParticleBuffer","particles")>] ParticleBuffer(gl: WebGLRenderingContext, properties: obj, dynamicPropertyFlags: ResizeArray<obj>, size: float) =
        member __.gl with get(): WebGLRenderingContext = jsNative and set(v: WebGLRenderingContext): unit = jsNative
        member __.vertSize with get(): float = jsNative and set(v: float): unit = jsNative
        member __.vertByteSize with get(): float = jsNative and set(v: float): unit = jsNative
        member __.size with get(): float = jsNative and set(v: float): unit = jsNative
    ...
    and [<AllowNullLiteral>] [<Import("ParticleRenderer","particles")>] ParticleRenderer(renderer: WebGLRenderer) =
        interface ObjectRenderer
        member __.shader with get(): glCore.GLShader = jsNative and set(v: glCore.GLShader): unit = jsNative
        member __.indexBuffer with get(): WebGLBuffer = jsNative and set(v: WebGLBuffer): unit = jsNative
...
module prepare =
    ...
    and [<AllowNullLiteral>] UploadHook<'UploadHookSource> =
        Func<'UploadHookSource, obj, bool>

    and [<AllowNullLiteral>] [<Import("BasePrepare","prepare")>] BasePrepare<'UploadHookSource>(renderer: SystemRenderer) =
    ...
    and [<AllowNullLiteral>] [<Import("CanvasPrepare","prepare")>] CanvasPrepare(renderer: CanvasRenderer) =
    ...
    and [<AllowNullLiteral>] [<Import("WebGLPrepare","prepare")>] WebGLPrepare(renderer: WebGLRenderer) =
        inherit BasePrepare<WebGLRenderer>()


    and [<AllowNullLiteral>] [<Import("CountLimiter","prepare")>] CountLimiter(maxItemsPerFrame: float) =
    ...
    and [<AllowNullLiteral>] [<Import("TimeLimiter","prepare")>] TimeLimiter(maxMilliseconds: float) =
...
module glCore =
    ...
    and [<AllowNullLiteral>] [<Import("GLBuffer","glCore")>] GLBuffer(gl: WebGLRenderingContext, ``type``: float, data: U3<ArrayBuffer, ArrayBufferView, obj>, drawType: float) =
    ...
    and [<AllowNullLiteral>] [<Import("GLFramebuffer","glCore")>] GLFramebuffer(gl: WebGLRenderingContext, width: float, height: float) =
        member __.gl with get(): WebGLRenderingContext = jsNative and set(v: WebGLRenderingContext): unit = jsNative
    ...
    and [<AllowNullLiteral>] [<Import("GLShader","glCore")>] GLShader(gl: WebGLRenderingContext, vertexSrc: U2<string, ResizeArray<string>>, fragmentSrc: U2<string, ResizeArray<string>>, ?precision: string, ?attributeLocations: obj) =
        member __.gl with get(): WebGLRenderingContext = jsNative and set(v: WebGLRenderingContext): unit = jsNative
    ...
    and [<AllowNullLiteral>] [<Import("GLTexture","glCore")>] GLTexture(gl: WebGLRenderingContext, ?width: float, ?height: float, ?format: float, ?``type``: float) =
        member __.gl with get(): WebGLRenderingContext = jsNative and set(v: WebGLRenderingContext): unit = jsNative
    ...
    and [<AllowNullLiteral>] [<Import("VertexArrayObject","glCore")>] VertexArrayObject(gl: WebGLRenderingContext, state: WebGLState) =
    ...
    type [<Import("*","glCore")>] Globals =
        ...
        static member setVertexAttribArrays(gl: WebGLRenderingContext, attribs: ResizeArray<Attrib>, ?state: WebGLState): U2<WebGLRenderingContext, undefined> = jsNative



module utils =
    type [<AllowNullLiteral>] [<Import("EventEmitter","utils")>] EventEmitter() =
...
module core =
    type [<AllowNullLiteral>] SpriteBatch =
        ParticleContainer

    and [<AllowNullLiteral>] AssetLoader =
        loaders.Loader

    and [<AllowNullLiteral>] Stage =
        Container

    and [<AllowNullLiteral>] DisplayObjectContainer =
        Container

    and [<AllowNullLiteral>] Strip =
        mesh.Mesh

    and [<AllowNullLiteral>] Rope =
        mesh.Rope

    and [<AllowNullLiteral>] ParticleContainer =
        particles.ParticleContainer

    and [<AllowNullLiteral>] MovieClip =
        extras.AnimatedSprite

    and [<AllowNullLiteral>] TilingSprite =
        extras.TilingSprite

    and [<AllowNullLiteral>] BaseTextureCache =
        obj

    and [<AllowNullLiteral>] BitmapText =
        extras.BitmapText

    and [<AllowNullLiteral>] math =
        obj

    and [<AllowNullLiteral>] AbstractFilter =
        Filter

    and [<AllowNullLiteral>] TransformManual =
        TransformBase

    and [<AllowNullLiteral>] TARGET_FPMS =
        float

    and [<AllowNullLiteral>] FILTER_RESOLUTION =
        float

    and [<AllowNullLiteral>] RESOLUTION =
        float

    and [<AllowNullLiteral>] MIPMAP_TEXTURES =
        obj

    and [<AllowNullLiteral>] SPRITE_BATCH_SIZE =
        float

    and [<AllowNullLiteral>] SPRITE_MAX_TEXTURES =
        float

    and [<AllowNullLiteral>] RETINA_PREFIX =
        U2<Regex, string>

    and [<AllowNullLiteral>] DEFAULT_RENDER_OPTIONS =
        float

    and [<AllowNullLiteral>] PRECISION =
        string



module extras =
    type [<AllowNullLiteral>] MovieClip =
        extras.AnimatedSprite



module settings =
    type [<AllowNullLiteral>] PRECISION =
        float
